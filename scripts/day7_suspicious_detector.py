...
def detect_suspicious(pcap_file):
    ...
    suspicious_ips = {ip: count for ip, count in syn_counts.items() if count > 10}
    ...
    with open("suspicious_report.txt","w") as f:
        f.write("SYN Packet Count (Potential Port Scan):\n")
        for ip,count in suspicious_ips.items():
            f.write(f"{ip}-> {count} SYNs\n")

detect_suspicious("C:/Users/Suzain/Desktop/NetSpy/captures/day7_Variety_capture.pcap")
